...
/// accept AND reject, plus the content_hash encoding (App C wins: field()-wrapped).
/// </summary>
public class PreImageTests
{
    ...
    public void Absent_session_emits_a_presence_byte_rather_than_being_omitted()
    {
        ...
        PreImage.Addr(without, new Address("bob", "gateway"));

        using var with = new MemoryStream();
        PreImage.Addr(with, new Address("bob", "gateway", "s1"));
    ...
    public void Null_reply_to_is_legal_and_emits_a_single_zero_presence_byte()
    {
        ...
        e.ReplyTo = new Address("bob", "gateway");
    ...
    [Theory]
    [InlineData("Uppercase")]        // uppercase — lowercasing is a MINT concern, never a silent fold here
    [InlineData("-bob")]       // leading hyphen
    [InlineData("bob-")]       // trailing hyphen
    [InlineData("wolf_x")]      // underscore is not LDH
    [InlineData("bob.gw")]     // dot is not LDH
    [InlineData("")]
