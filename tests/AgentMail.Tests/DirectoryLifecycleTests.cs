...
/// <summary>
/// Regression tests for #8 — the directory-lifecycle bug that took an agent off the mesh (its own relay 404'd it) on
/// 2026-07-17 at 24.2h and made its inbound flap 202/404 on a ~60s cycle.
///
...
public class DirectoryLifecycleTests
{
    ...
    {
        // THE INCIDENT. an agent's own relay 404'd it at 24.2h because its record aged out of its own directory.
        // Resolve(name, Paths.Host) failing for a local agent is incoherent: a relay always knows what it hosts.
        var local = Rec("alice", Paths.Host, HoursAgo(999));
    ...
    {
        var local = Rec("alice-prunetest", Paths.Host, HoursAgo(999));
    ...
    public void Deleting_a_record_destroys_the_LWW_high_water_so_a_re_register_can_lose_to_a_stale_ghost()
    {
        ...
        // accidentally preserved bob's version when he re-registered.
        var ghostFromPeer = Rec("bob", "some-peer-host", HoursAgo(30), version: 3);
        var freshAfterDeletion = Rec("bob", "some-peer-host", Now, version: 1);   // version reset by the delete
    ...
    {
        // A required assertion. Status is self-asserted at register and never refreshed, so it cannot go
    ...
    public void A_fresh_record_is_routable_and_an_explicit_tombstone_is_not()
    {
        ...
        Assert.True(DirectoryStore.IsRoutable(Rec("peer", "host-x", HoursAgo(0.2)), Now));
        Assert.False(DirectoryStore.IsRoutable(Rec("retired", "host-x", HoursAgo(0.2), status: "offline"), Now));
    }

    [Theory]
    [InlineData(0.1, false)]
    [InlineData(23.9, false)]
    [InlineData(24.2, true)]    // bob's exact age at the incident
