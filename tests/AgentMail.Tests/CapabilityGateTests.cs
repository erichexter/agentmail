...
public class CapabilityGateTests
{
    ...
    {
        // Wolf's secondbrain case: a partitioned peer never converges, but grace elapses, so its plaintext is
        // quarantined (fail closed). The decision returns immediately — no blocking on the partition.
        var recipient = Save("e2ercpt", e2e: true, lastSeen: Base);
        var partitioned = Save("secondbrain", e2e: true, lastSeen: Base.AddMinutes(-30));  // old, isolated
